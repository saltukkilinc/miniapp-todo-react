...
const NewUpdateTodo = ({setOpen}) => {
  ...
  useEffect(() => {
    setUpdateInput(ctx.updateInputValue())
  }, [isActive])


  const updateButtonHandler = () => {
    ctx.updateHandler(updateInput)
    setUpdateInput('')
  }
  ...
  return (
    <>
        ...
        <label onClick={ctx.addPinHandler}>
          ...
          {isActive && (
            <svg
              ...
              xmlns="http://www.w3.org/2000/svg"
            >
              <circle cx="6" cy="6" r="5.5" fill="#FF7964" stroke="#FF7964" />
            </svg>
          )}
        </label>
      </div>
      ...
      <div className={styles.buttonContainer}>
        <button className={styles.saveButton} onClick={updateButtonHandler}>Update</button>
